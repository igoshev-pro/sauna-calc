...
import { WorkTypeForm } from '@/components/work-types/WorkTypeForm';
...
export default function WorkTypesPage() {
  ...
  const [editItem, setEditItem] = useState<WorkType | null>(null);
  ...
  const handleEdit = (it: WorkType) => { setEditItem(it); setFormOpen(true); };

  const handleSubmit = async (data: Partial<WorkType>) => {
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden flex-1">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 gap-3">
              ...
              <p className="text-sm text-gray-500">Виды работ не найдены</p>
              {isAdmin && (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    ...
                    <th className="text-right px-4 py-3 font-medium text-gray-600">Наценка</th>
                  ...
                  {items.map((it) => (
                    <tr key={it._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right text-gray-900">
                        {it.laborCostPerUnit.toLocaleString('ru-RU')} ₽
                      </td>
                      <td className="px-4 py-3 text-right text-gray-600">{it.markupPercent}%</td>
                      <td className="px-4 py-3 text-right text-gray-600">
                        {it.materialFormulas?.length ?? 0}
                      </td>
                      {isAdmin && (
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)}
        title={editItem ? 'Редактировать вид работ' : 'Добавить вид работ'} size="lg">
        <WorkTypeForm
          initial={editItem}
